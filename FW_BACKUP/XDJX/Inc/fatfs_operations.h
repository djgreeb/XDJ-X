...
if(track_play_now[dkN]!=0)
	...
	if(need_seek[dkN]!=0)
		...
		if(need_seek[dkN]==1)
			{
			if((play_adr[dkN]+seek_pos[dkN])<(all_long[dkN]*294))	
				{
				SEEK_AUDIOFRAME(dkN, play_adr[dkN]+seek_pos[dkN]);	
				}				
			}
		else if(need_seek[dkN]==2)
			{	
			if(play_adr[dkN]>seek_pos[dkN])	
				{
				SEEK_AUDIOFRAME(dkN, play_adr[dkN]-seek_pos[dkN]);		
				}	
			}	
		else if(need_seek[dkN]==3)
			{		
			SEEK_AUDIOFRAME(dkN, seek_pos[dkN]);	
			}
		need_seek[dkN] = 0;								
		}			
	}
